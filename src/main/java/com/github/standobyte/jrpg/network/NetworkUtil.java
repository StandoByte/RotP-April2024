...
public class NetworkUtil {
    ...
    public static void writeEffectInstance(PacketBuffer buf, EffectInstance effectInstance) {
        byte effectId = (byte) (Effect.getId(effectInstance.getEffect()) & 255);
        byte effectAmplifier = (byte) (effectInstance.getAmplifier() & 255);
        int effectDurationTicks;
        if (effectInstance.getDuration() > 32767) {
           effectDurationTicks = 32767;
        } else {
           effectDurationTicks = effectInstance.getDuration();
        }
        ...
        buf.writeByte(effectAmplifier);
        buf.writeVarInt(effectDurationTicks);
        buf.writeByte(flags);
    }
    
    public static EffectInstance readEffectInstance(PacketBuffer buf) {
        byte effectId = buf.readByte();
        byte effectAmplifier = buf.readByte();
        int effectDurationTicks = buf.readVarInt();
        byte flags = buf.readByte();
        
        boolean isEffectVisible = (flags & 2) == 2;
        boolean isEffectAmbient = (flags & 1) == 1;
        boolean effectShowsIcon = (flags & 4) == 4;
        
        Effect effect = Effect.byId(effectId & 0xFF);
        if (effect != null) {
            EffectInstance effectInstance = new EffectInstance(effect, effectDurationTicks, effectAmplifier, isEffectAmbient, isEffectVisible, effectShowsIcon);
            effectInstance.setNoCounter(effectDurationTicks == 32767);
    ...
    public static <T> void writeStream(PacketBuffer buf, Stream<T> stream, Consumer<T> writeElement) {
        ...
        buf.setInt(initialWriterIndex, i.intValue());
    }
